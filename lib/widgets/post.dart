...
class _PostState extends State<Post> {
    ...
    return FutureBuilder(
      ...
      builder: (context, snapshot) {
        ...
        return ListTile(
          ...
          trailing: isPostOwner ? IconButton(                   // only owner can delete its post
            onPressed: () => handleDeletePosts(context),
            icon: Icon(Icons.more_vert),
            // otherwise don't show this IconButton
          ) : Text(''),
        );
      },
    );
  ...
  handleDeletePosts(BuildContext parentContext) {
    return showDialog(
  ...
  // ownerId is equal to currentUserId, so they can be used interchangeably
  deletePost() async {
    // delete post itself
    postsRef
      .document(ownerId)
  ...
  addLikeToActivityFeed () {
    // add a notification in a postOwner's activity feed only if like made by other user
  ...
  buildPostImage() {
    return GestureDetector(
      ...
      child: Stack(
        ...
        children: <Widget>[
          cachedNetworkImage(mediaUrl),     // image itself
          // animated heartbeat (if double tap on the post)
          showHeart ? Animator(duration: Duration(milliseconds: 300),
      ...
      children: <Widget>[
        Row(
          ...
          children: <Widget>[
            ...
            GestureDetector(                                        // comments button
              onTap: () => showComments(
                context, 
                postId: postId,
                ownerId: ownerId,
        ...
        Row(
          children: <Widget>[
            Container(
              ...
              child: Text(
                "$likeCount likes", style: TextStyle(
                  color: Colors.black,
                  fontWeight: FontWeight.bold,
                ),
              ),
...
showPost(context, { String postId, String userId} ) {
  Navigator.push(context, MaterialPageRoute(
