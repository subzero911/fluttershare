...
class _TimelineState extends State<Timeline> {
  ...
  getFollowing() async {
    QuerySnapshot snapshot = await followingRef
      .document(currentUser.id).collection('userFollowing').getDocuments();
    setState(() {
  ...
  buildTimeline() {
    if (posts == null) return circularProgress();
    else if (posts.isEmpty) return buildUsersToFollow();
    else return ListView(children: posts);
  }

  // we use stream because we want to track users that sign up in realtime
  buildUsersToFollow() {
    return StreamBuilder(
      ...
      builder: (context, snapshot) {
        ...
        snapshot.data.documents.forEach((doc) {
          ...
          else if (isFollowingUser) return;
          else {
        ...
        return Container(
          ...
          child: Column(
            children: <Widget>[
              ...
              userResults.isEmpty ? Text("No users registered") : Column(children: userResults)
            ],
          ),
        );
      },
